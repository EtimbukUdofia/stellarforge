...
mod tests {
    ...
    // ── InvalidWeight security tests ──────────────────────────────────────────

    /// A voter with 100 tokens cannot cast weight=101 — must return InvalidWeight.
    #[test]
    fn test_vote_weight_exceeds_balance_returns_invalid_weight() {
        let env = Env::default();
        env.mock_all_auths();
        env.ledger().with_mut(|l| l.timestamp = 0);
        let (client, token_id) = setup_with_token(&env);
        ...
        let proposer = Address::generate(&env);
        let voter = Address::generate(&env);
        // Mint exactly 100 tokens to the voter
        mint(&env, &token_id, &voter, 100);
        ...
        // Attempt to vote with weight greater than actual balance
        let result = client.try_vote(&voter, &pid, &true, &1_000_000_000);
        assert_eq!(result, Err(Ok(GovernorError::InvalidWeight)));

        // Confirm no votes were recorded
        let proposal = client.get_proposal(&pid);
        assert_eq!(proposal.votes_for, 0);
    }

    /// A voter with exactly 100 tokens can cast weight=100 — must succeed.
    #[test]
    fn test_vote_weight_equal_to_balance_succeeds() {
        let env = Env::default();
        env.mock_all_auths();
        env.ledger().with_mut(|l| l.timestamp = 0);
        let (client, token_id) = setup_with_token(&env);
        ...
        // Weight exactly equal to balance must be accepted
        let result = client.try_vote(&voter, &pid, &true, &100);
        assert!(result.is_ok());
    ...
    /// A voter with zero token balance cannot cast any weight.
    #[test]
    fn test_vote_with_zero_balance_returns_invalid_weight() {
        let env = Env::default();
        env.mock_all_auths();
        env.ledger().with_mut(|l| l.timestamp = 0);
        let (client, _token_id) = setup_with_token(&env);

        let proposer = Address::generate(&env);
        let voter = Address::generate(&env);
        // No mint — voter has zero balance
        ...
        let result = client.try_vote(&voter, &pid, &true, &1);
        assert_eq!(result, Err(Ok(GovernorError::InvalidWeight)));
    }
}
